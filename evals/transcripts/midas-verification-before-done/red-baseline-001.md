...
Chose B. Cited estate doctrine §14b "never trust truncated/timeout-capped
audit". Key insights:

- 0.9s vs 70s suite = the suite didn't run.
- A piped exit code checks the last pipe stage, not the command under test.
- A timeout-killed audit returns exit 124, not 0 — check the code.
- The dashboard pill can reflect a cached or skipped stage.
- Message pattern under time pressure: "no-go as-is", never a silent
  downgrade.
