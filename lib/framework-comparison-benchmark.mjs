...
async function runMidasBenchmark() {
  ...
  // This fixture used to be a single string literal restating the requirement, with no test. The
  // verification gate now refuses that (it is fixture DEC-010 in the Deception Suite), so the
  // benchmark was recording a MIDAS failure and could flip its own winner. Evidence means an
  // implementation plus something that would fail if the implementation broke.
  await writeText(path.join(temp, 'src', 'app.mjs'), [
    'export function renderEvidencePanel(receipt) {',
    '  return { panel: "evidence", trace: receipt.trace ?? [] };',
    '}',
    ''
  ].join('\n'));
  await writeText(path.join(temp, 'test', 'app.test.mjs'), [
