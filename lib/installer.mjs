...
function adapterBody(tool, modules, policy, agentProfiles) {
...
function adapterText(tool, modules, policy, agentProfiles) {
  // MIDAS.md keeps the full adapter body for one release so existing references
  // and adapter contracts keep working while harnesses migrate to SKILL.md.
  return `> DEPRECATED: this MIDAS.md is superseded by the SKILL.md beside it, which
> harness-native skill discovery loads automatically. MIDAS.md remains for one
> release so existing references keep working, then stops being generated.

${adapterBody(tool, modules, policy, agentProfiles)}`;
}

function adapterSkillText(tool, modules, policy, agentProfiles) {
  // SKILL.md is the discoverable form: harnesses with native skill support load
  // it by frontmatter instead of needing an instruction to read MIDAS.md.
  return `---
name: midas
description: Use when working in this repository — loads the MIDAS operating contract, load order, active modules, agent profiles, and permission policy before acting.
license: Apache-2.0
...
The installed MIDAS package ships a SessionStart bootstrap under its hooks/
directory (hooks/hooks.json runs node hooks/session-start.mjs on startup, clear,
and compact). To wire it:

- Claude Code: merge the SessionStart entry from the package's hooks/hooks.json
  into your settings hooks configuration, keeping the command path resolvable
  from the session working directory.
- Other harnesses: run node hooks/session-start.mjs from the package root at
  session start and prepend the emitted additionalContext, or read
  .midas/skills/using-midas/SKILL.md directly as startup context.
`;
}
...
export async function installWorkspace(options) {
  ...
  for (const tool of tools) {
    ...
    const skillFile = path.join(adapterDir, 'SKILL.md');
    await fs.writeFile(skillFile, adapterSkillText(tool, modules, policy, agentProfiles));
