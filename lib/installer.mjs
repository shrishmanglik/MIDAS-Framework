...
import { mergeMidasBlock } from './adapter-markers.mjs';
...
function adapterSkillText(tool, modules, policy, agentProfiles) {
  // Only call this for a loader-verified native SKILL.md path. Other adapters keep
  // their actual loader contract instead of receiving an inert lookalike skill.
  return `---
name: midas
...
export async function installWorkspace(options) {
  ...
  for (const tool of tools) {
    const relative = adapterFilePath(tool);
    ...
    const body = tool === 'claude-code'
      ? adapterSkillText(tool, modules, policy, agentProfiles)
      : adapterBody(tool, modules, policy, agentProfiles);

    if (adapterMode(tool) === 'merge') {
      // Shared file (AGENTS.md): never clobber the project's own guidance.
      let existing = '';
      try {
      ...
      await fs.writeFile(file, mergeMidasBlock(existing, body));
    } else {
