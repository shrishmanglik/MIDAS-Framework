...
// Claude Code only loads a skill when SKILL.md carries YAML frontmatter with `name`
// and `description`; the description is what it matches against to decide relevance.
function claudeSkillFrontmatter() {
  return `---
name: midas
description: Use the MIDAS workspace that governs this project. Read .midas/project-context.md, the manifest, and sprint status before changing files, keep work inside one scoped work order, respect the approval gates, and record evidence before claiming completion. Use when planning, implementing, reviewing, or closing out work in a MIDAS project.
---
...
export async function installWorkspace(options) {
  ...
  for (const tool of tools) {
    ...
    const body = adapterText(tool, modules, policy, agentProfiles);
    ...
    if (adapterMode(tool) === 'merge') {
      ...
      await fs.writeFile(file, `${claudeSkillFrontmatter()}${body}`);
    } else {
