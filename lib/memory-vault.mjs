...
// Frontmatter is line-oriented, so a newline in any single-line meta field would
// inject or corrupt fields. Collapse all whitespace to a single space.
...
export async function addMemory({ directory = '.', name, description, type = 'project', body = '', tags = [] }) {
  ...
  const entry = {
    name: slug,
    description: sanitizeMetaLine(description),
    type,
    created,
    updated: now,
    tags: (Array.isArray(tags) ? tags : String(tags).split(','))
      .map((tag) => sanitizeMetaLine(tag).replace(/,/g, ' ').trim())
      .filter(Boolean),
