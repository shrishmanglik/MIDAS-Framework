...
const requiredInvariantIds = [
  ...
  'approval-gates',
  'tdd-negative-test-must-fail-prefix',
...
const allowedInvariantVerification = new Set([
  ...
  'negative-test-red-run',
  'done-claim-evidence-check',
  'reviewer-session-check'
]);
const allowedLoopEscalations = new Set(['route-up-on-cap']);
const loopPolicyMaxRoundsCeiling = 10;
...
function validateLoopPolicy(constitution, file, failures) {
  const policy = constitution.loopPolicy;
  if (policy === undefined) return;
  if (!isPlainObject(policy)) {
    failures.push(failure('authority:constitution:loop-policy', file, 'loopPolicy must be an object when present.'));
    return;
  }
  unknownFields(policy, new Set(['fixLoopMaxRounds', 'escalation']), 'constitution', 'loop-policy', file, failures);
  if (!Number.isInteger(policy.fixLoopMaxRounds) || policy.fixLoopMaxRounds < 1 || policy.fixLoopMaxRounds > loopPolicyMaxRoundsCeiling) {
    failures.push(failure(
      'authority:loop-policy:fix-loop-max-rounds',
      file,
      `loopPolicy.fixLoopMaxRounds must be an integer between 1 and ${loopPolicyMaxRoundsCeiling}.`
    ));
  }
  if (typeof policy.escalation !== 'string' || !allowedLoopEscalations.has(policy.escalation)) {
    failures.push(failure(
      'authority:loop-policy:escalation',
      file,
      `loopPolicy.escalation must be one of: ${[...allowedLoopEscalations].join(', ')}.`
    ));
  }
}
...
export function validateAuthorityContracts(constitution, file = 'authority constitution') {
  ...
  validateLoopPolicy(constitution, file, failures);
  return failures;
}
...
export async function inspectAuthorityContracts(directory, options = {}) {
  ...
  return {
    ...
    loopPolicy: failures.length === 0 ? constitution.loopPolicy ?? null : null,
    failures
  };
}
