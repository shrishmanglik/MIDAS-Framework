  ...
  if (provider === 'manual') {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        record({ type: 'tool-result', step: steps, tool: '(parse)', ok: false, error: `Malformed JSONL action on line ${steps}; skipped.` });
        continue;
      }
      if (!action || typeof action.tool !== 'string') {
        record({ type: 'tool-result', step: steps, tool: '(parse)', ok: false, error: 'Action missing a "tool" string; skipped.' });
        continue;
      }
