...
import { generateLicenseKeypair, installLicense, licenseStatus, signLicense } from './license.mjs';
import { startMcpServer } from './mcp-server.mjs';
import { addMemory, inspectMemoryLinks, listMemories, searchMemories, showMemory } from './memory-vault.mjs';
...
Commands:
  ...
  mcp          Start the MIDAS MCP stdio server (tools for MCP-capable agents).
  memory       Manage the .midas/memory vault: add | list | search | show | links.
  web          Fetch a public URL with text extraction and an evidence receipt: web fetch <url>.
  agent        Run a bounded agent loop: --objective "..." --provider manual|script|anthropic-api.
  license      License scaffold: status | install | keygen | sign.
...
Examples:
  ...
  midas mcp --directory ./my-project
  midas memory add --directory ./my-project --name auth-flow --description "How auth works" --body "Supabase JWT. See [[deploy-notes]]."
  midas memory search --directory ./my-project "auth"
  midas web fetch https://example.com --save
  midas agent --directory ./my-project --objective "Summarize repo health" --provider anthropic-api
  midas license status --directory ./my-project
`);
}
...
export async function runCli(argv) {
  ...
  if (command === 'mcp') {
    startMcpServer({ directory: parsed.directory ?? '.' });
    return;
  }
  ...
  if (command === 'memory') {
    ...
    let result;
    if (sub === 'add') {
      result = await addMemory({
        directory,
        name: parsed.name,
        description: parsed.description ?? parsed._.slice(1).join(' '),
        type: parsed.type ?? 'project',
        body: parsed.body ?? '',
        tags: parsed.tags ?? []
      });
    } else if (sub === 'search') {
      result = await searchMemories({ directory, query: parsed.query ?? parsed._.slice(1).join(' '), limit: parsed.limit });
    } else if (sub === 'show') {
      result = await showMemory({ directory, name: parsed.name ?? parsed._[1] });
    } else if (sub === 'links') {
      result = await inspectMemoryLinks({ directory });
    } else {
      result = await listMemories({ directory });
    }
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  ...
  if (command === 'web') {
    ...
    if (sub !== 'fetch') throw new Error('web supports: fetch <url>');
    const result = await fetchWeb({
      directory: parsed.directory ?? '.',
      url: parsed.url ?? parsed._[1],
      render: Boolean(parsed.render),
      save: Boolean(parsed.save),
      allowLocal: Boolean(parsed['allow-local']),
      maxBytes: parsed['max-bytes'] ? Number(parsed['max-bytes']) : undefined,
      timeoutMs: parsed.timeout ? Number(parsed.timeout) : undefined
    });
    console.log(JSON.stringify(result, null, 2));
    if (result.status !== 'pass') process.exitCode = 1;
    return;
  ...
  if (command === 'agent') {
    const result = await runAgentLoop({
      directory: parsed.directory ?? '.',
      objective: parsed.objective ?? parsed._.join(' '),
      provider: parsed.provider ?? 'manual',
      model: parsed.model,
      maxSteps: parsed['max-steps'] ? Number(parsed['max-steps']) : undefined,
      allowWrites: Boolean(parsed['allow-writes']),
      allowWeb: Boolean(parsed['allow-web']),
  ...
  if (command === 'license') {
    const sub = parsed._[0] ?? 'status';
    const directory = parsed.directory ?? '.';
    let result;
    if (sub === 'keygen') {
      result = await generateLicenseKeypair({ out: parsed.out ?? '.' });
    } else if (sub === 'sign') {
      result = await signLicense({
      ...
      result = await installLicense({ directory, file: parsed.file ?? parsed._[1] });
    } else {
      result = await licenseStatus({ directory, publicKey: parsed['public-key'] });
    }
    console.log(JSON.stringify(result, null, 2));
    return;
  }
