...
import { inspectWorkflowLibrary } from './workflow-library.mjs';
...
const internalWorkReferenceTerms = [
  phrase(['dev', 'studio'], '-'),
  phrase(['operating', 'estate']),
  phrase(['estate', 'CEO'])
];

const internalWorkIdPrefixes = [
  phrase(['MIDAS', 'BRIDGE'], '-'),
  'MS',
  'MD'
];

const internalWorkReferencePattern = new RegExp(
  `(?:\\b(?:${internalWorkIdPrefixes.map(escapePattern).join('|')})-\\d+\\b|\\b(?:${internalWorkReferenceTerms.map(escapePattern).join('|')})\\b)`,
  'i'
);

const forbiddenPatterns = [
  { id: 'private-root-path', pattern: /\b[A-Z]:(?:\/|\\)[^\r\n]*(?:private|internal|confidential|client|customer)[^\r\n]*(?:\/|\\)/i, severity: 'high' },
  { id: 'machine-local-path', pattern: /\b[A-Z]:(?:\/|\\)(?:Users(?:\/|\\)|[^/\\\r\n<>:"'|?*,]* [^/\\\r\n<>:"'|?*,]*(?:\/|\\))/, severity: 'high' },
  { id: 'secret-assignment', pattern: /\b(api[_-]?key|secret[_-]?key|private[_-]?key|access[_-]?token|refresh[_-]?token)\s*(?:=\s*["'][^"'\r\n]{8,}["']|:\s*["'][^"'\r\n]{8,}["'])/i, severity: 'critical' },
  ...
  { id: 'internal-work-reference', pattern: internalWorkReferencePattern, severity: 'medium' }
];
...
// Harness configuration directories are commonly committed and published with a
// repo, so the public-boundary scan must descend into them. Other dot
// directories stay excluded to avoid scanning caches and local editor state.
const scannedDotDirectories = new Set([
  '.github',
  '.claude',
  '.codex',
  '.cursor',
...
async function walk(root, files = []) {
  ...
  for (const entry of entries) {
    if (entry.name.startsWith('.') && !(entry.isDirectory() && scannedDotDirectories.has(entry.name))) continue;
...
async function validateRepository(target) {
  ...
  const required = [
    ...
    'framework/skills/verification-gap/SKILL.md',
    'framework/skills/spine-ux/SKILL.md',
    'framework/skills/local-planning/SKILL.md',
    'framework/modules/module.schema.json',
    'framework/modules/agentic-agile/module.json',
    'framework/templates/work-order-template.md',
    'framework/templates/project-context-template.md',
    'framework/templates/prd-template.md',
    'framework/templates/design-template.md',
    'framework/templates/experience-template.md',
    'framework/templates/verification-gap-receipt-template.md',
  ...
  try {
    const result = await inspectWorkflowLibrary(target);
    checks.push({
      id: 'workflow-library:default',
      status: result.status,
      remediation: result.status === 'pass' ? '' : 'Fix midas.workflow.v2 files under framework/workflows.'
    });
    for (const failure of result.failures) {
      checks.push(failure);
    }
    ...
    checks.push({
      id: 'workflow-library:default',
      status: 'fail',
      remediation: `Unable to inspect default workflow library: ${error.message}`
    });
  }
