...
import { extractMidasBlock } from './adapter-markers.mjs';
...
function inspectAdapterText(contract, fileText) {
  const failures = [];
  // When MIDAS shares a file with the project (AGENTS.md), validate only the
  // MIDAS-owned block. The project's own guidance is not ours to police, and it
  // must not consume our size budget.
  const text = extractMidasBlock(fileText) ?? fileText;
